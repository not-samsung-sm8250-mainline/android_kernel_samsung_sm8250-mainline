 ...
 *
 * This syntax matches dm-crypt, but unneeded functionality has been removed.
 */
static int default_key_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (bdev_is_zoned(dkc->dev->bdev)) {
		/* IVs based on sectors require zone append emulation. */
...
static int default_key_map(struct dm_target *ti, struct bio *bio)
{
	...
	/* Encrypted file contents already carry their own fscrypt key. */
	...
	/*
	 * The bio may need blk-crypto-fallback processing.  Call the lower-level
	 * helper so DM can retain the correct ownership and accounting.
	 */
				     ...
				     bool *forward)
{
	...
	*bdev = dev->bdev;
	...
	if (dkc->start != 0 || ti->len != bdev_nr_sectors(dev->bdev))
		return 1;
	return 0;
}
